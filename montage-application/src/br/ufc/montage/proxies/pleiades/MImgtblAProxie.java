...
public class MImgtblAProxie extends MontageShelfComputationComponent{
	...
	public void setServices(IHShelfService services) {
		...
		this.goPortTask.setName("mimgtbl-go-A");
		
		try {
			
			this.services.registerTaskPort(goPortTask);
			this.services.registerUsesPort("mimgtbl-raw-dir-port-uses-A", MontageTypes.DIR_TYPE);
			this.services.registerUsesPort("mimgtbl-proj-dir-port-uses-A", MontageTypes.DIR_TYPE);
		...
		this.tblPortProvides.setName("mimgtbl-tbl-port-provides-A");
		try {
	...
	public void go() {
		...
		try {
			...
			if(turn==0){
				this.rawDirPortUses = (HShelfUsesPort)this.services.getPort("mimgtbl-raw-dir-port-uses-A");
				dirPortValue = ((MontageShelfProvidesPort)this.rawDirPortUses.getProvidesPort()).getValue();
				turn = 1;
			}else{
				this.projDirPortUses = (HShelfUsesPort)this.services.getPort("mimgtbl-proj-dir-port-uses-A");
