...
public class MontageWorkflow implements Serializable {
	...
	public List<MontageEnvConnection> listMontageEnvConnectionsByActPosition(
			Integer actPosition) {
		return this.envConnPosMap.get(actPosition);
	}

	public List<MontageTskConnection> listMontageTskConnectionsByActPosition(
			Integer actPosition) {
	...
	public void deleteConnections(Integer actPosition, String compName) {
		...
		List<MontageEnvConnection> envConns = this
				.listMontageEnvConnectionsByActPosition(actPosition);
		if (envConns != null) {
			List<MontageEnvConnection> indexes = new ArrayList<MontageEnvConnection>();
			
			for (MontageEnvConnection env : envConns) {
				if (env.getUsesCompName().equals(compName)) {
					indexes.add(env);
				}
			}
			
			for (MontageEnvConnection env : indexes) {
				envConns.remove(env);
			}
		...
		List<MontageTskConnection> tskConns = this
				.listMontageTskConnectionsByActPosition(actPosition);
		if (tskConns != null) {
			List<MontageTskConnection> indexes = new ArrayList<MontageTskConnection>();
			...
			for (MontageTskConnection tsk : indexes) {
				tskConns.remove(tsk);
			}
		}
	}
